export const empresasColaboradoras = [
    {
        nombre: "AGUILAS GOLF",
        logo: "/img-empresas/aguilas-golf.webp",
        url: "https://aguilas.golf/",
        es: {
            descripcion: [
                "La Suerte Favorece a los Preparados: Practica Tu Juego de Golf en Águilas Hoy.",
                "Adquiere Ventaja sobre el Juego Mejorando tu Swing con Práctica en nuestra Academia de Golf.",
                "La Suerte Favorece a los Preparados: Practica Tu Juego de Golf Hoy en la Academia de Isla del Fraile.",
                "En resumen, jugar al golf no es solo un deporte, sino una forma de vida que combina deporte, entretenimiento, desafío y relajación."
            ],
            btnText: "¡RESERVA YA!"
        },
        en: {
            descripcion: [
                "Luck Favors the Prepared: Practice Your Golf Game in Águilas Today.",
                "Gain an Edge in the Game by Improving your Swing with Practice at our Golf Academy.",
                "Luck Favors the Prepared: Practice Your Golf Game Today at the Isla del Fraile Academy.",
                "In short, playing golf is not just a sport, but a lifestyle combining sport, entertainment, challenge, and relaxation."
            ],
            btnText: "BOOK NOW!"
        },
        de: {
            descripcion: [
                "Das Glück begünstigt die Vorbereiteten: Üben Sie heute Ihr Golfspiel in Águilas.",
                "Verschaffen Sie sich einen Vorteil, indem Sie Ihren Schwung in unserer Golfakademie verbessern.",
                "Das Glück begünstigt die Vorbereiteten: Üben Sie heute Ihr Golfspiel in der Akademie der Isla del Fraile.",
                "Kurz gesagt, Golf zu spielen ist nicht nur ein Sport, sondern eine Lebensweise, die Sport, Unterhaltung, Herausforderung und Entspannung vereint."
            ],
            btnText: "JETZT BUCHEN!"
        }
    },
    {
        nombre: "AGUILAS HOSTEL",
        logo: "/img-empresas/aguilas-hostel.webp",
        url: "http://www.hostelaguilas.com",
        es: {
            descripcion: [
                "¡Albergue - Hostel situado en Murcia con ofertas exclusivas, en una ubicación en urbanización de lujo!",
                "Busca y reserva albergue barato en línea. Próxima parada: El alojamiento en Murcia adecuado para ti.",
                "Gran variedad de opciones en un HOSTEL DE PRIMERA CATEGORÍA.",
                "Alquiler de Habitaciones Albergue Águilas 7 días a la semana 365 días del año ¿Quieres Más? Contáctanos!"
            ],
            btnText: "¡RESERVA YA!"
        },
        en: {
            descripcion: [
                "Hostel located in Murcia with exclusive offers, in a luxury residential location!",
                "Search and book cheap hostel online. Next stop: The right accommodation in Murcia for you.",
                "Wide variety of options in a FIRST CATEGORY HOSTEL.",
                "Room Rental Águilas Hostel 7 days a week, 365 days a year. Want More? Contact us!"
            ],
            btnText: "BOOK NOW!"
        },
        de: {
            descripcion: [
                "Hostel in Murcia mit exklusiven Angeboten, in einer luxuriösen Wohnlage!",
                "Suchen und buchen Sie günstige Hostels online. Nächste Station: Die richtige Unterkunft in Murcia für Sie.",
                "Große Auswahl in einem ERSTKLASSIGEN HOSTEL.",
                "Zimmervermietung Águilas Hostel 7 Tage die Woche, 365 Tage im Jahr. Möchten Sie mehr? Kontaktieren Sie uns!"
            ],
            btnText: "JETZT BUCHEN!"
        }
    },
    {
        nombre: "PALAZÓN A&S",
        logo: "/img-empresas/palazon.webp",
        url: "https://palazonasociados.com/",
        es: {
            descripcion: [
                "Un despacho multidisciplinar formado por profesionales del derecho que ofrece asesoramiento integral tanto a particulares como a empresas.",
                "Actualmente cuenta con dos despachos, Murcia y Águilas, donde ofrecer a cada uno de sus clientes sus servicios jurídicos y el compromiso de un trabajo personalizado.",
                "Siempre para garantizar unos resultados satisfactorios avalados por los años de experiencia en ámbitos del derecho civil, social, penal e internacional privado."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "A multidisciplinary firm formed by legal professionals offering comprehensive advice to both individuals and companies.",
                "Currently has two offices, Murcia and Águilas, where we offer clients legal services and the commitment to personalized work.",
                "Always guaranteeing satisfactory results backed by years of experience in civil, social, criminal, and private international law."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Eine multidisziplinäre Kanzlei aus Juristen, die umfassende Beratung für Privatpersonen und Unternehmen bietet.",
                "Derzeit verfügt die Kanzlei über zwei Büros in Murcia und Águilas, wo sie ihren Mandanten Rechtsdienstleistungen und persönliche Betreuung anbietet.",
                "Stets für zufriedenstellende Ergebnisse, gestützt auf jahrelange Erfahrung im Zivil-, Sozial-, Straf- und internationalen Privatrecht."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    },
    {
        nombre: "BIENHABLAO",
        logo: "/img-empresas/bienhablao.webp",
        url: "https://vinosbienhablao.com/",
        es: {
            descripcion: [
                "El proyecto mas personal de Pedro Muñoz Carrasco nace con la selección de las variedades autóctonas de la zona del sureste de la península Ibérica, el Levante español se caracteriza por sus vinos de gran color y gran aroma afrutado.",
                "La Monastrell como variedad predominante de los viñedos de esta zona, y de origen español, nos ofrece una cepa de porte erguido con sarmientos gruesos y cortos, entrenudos de longitud media y poco ramificados; la hoja posee un limbo de forma pentagonal, con tres lóbulos marcados."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "Pedro Muñoz Carrasco's most personal project was born from selecting native varieties from the southeastern Iberian Peninsula. The Spanish Levante region is characterized by wines of great color and fruity aromas.",
                "Monastrell, as the predominant variety in vineyards of this area, offers an upright vine with thick and short shoots, internodes of medium length and slightly branched; the leaf has a pentagonal blade, with three marked lobes."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Das persönlichste Projekt von Pedro Muñoz Carrasco entstand mit der Auswahl einheimischer Rebsorten aus dem Südosten der Iberischen Halbinsel. Der spanische Levanten-Bereich zeichnet sich durch seine Weine mit intensiver Farbe und fruchtigem Aroma aus.",
                "Die Monastrell als vorherrschende Rebsorte der Weinberge in dieser Region bietet eine aufrechte Rebe mit dicken und kurzen Ranken, mittellagen Internodien und wenig verzweigt; das Blatt hat ein fünfeckiges Blatt mit drei markierten Lappen."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    },
    {
        nombre: "CAPITÁN BUZO",
        logo: "/img-empresas/capitan-buzo.webp",
        url: "https://www.capitanbuzo.com",
        es: {
            descripcion: [
                "Capitán Buzo Club es una empresa especializada en formación y experiencias de buceo, que ofrece cursos para todos los niveles, bautismos, salidas y viajes organizados.",
                "Trabajan con total profesionalidad y seguridad, permitiendo descubrir el mundo submarino de una forma emocionante y accesible.",
                "Una propuesta diferente para vuestros clientes de pádel, combinando deporte, aventura y naturaleza."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "Capitán Buzo Club is a company specialized in diving training and experiences, offering courses for all levels, baptisms, outings, and organized trips.",
                "They work with total professionalism and safety, allowing you to discover the underwater world in an exciting and accessible way.",
                "A different proposal for your pickleball clients, combining sport, adventure, and nature."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Capitán Buzo Club ist ein Unternehmen, das auf Tauchausbildung und -erlebnisse spezialisiert ist und Kurse für alle Niveaus, Schnuppertauchen, Ausflüge und organisierte Reisen anbietet.",
                "Sie arbeiten mit höchster Professionalität und Sicherheit und ermöglichen es, die Unterwasserwelt auf aufregende und zugängliche Weise zu entdecken.",
                "Ein andersartiges Angebot für Ihre Pickleball-Kunden, das Sport, Abenteuer und Natur verbindet."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    },
    {
        nombre: "GERMÁN SÁEZ",
        logo: "/img-empresas/german-saez.webp",
        url: "https://www.germansaez.com.es/",
        es: {
            descripcion: [
                "En Asesoría Germán Sáez, transformamos números en estrategias exitosas.",
                "Desde la contabilidad de sociedades y costes hasta el detallado análisis de amortizaciones.",
                "Nuestra experiencia abarca el Impuesto de Sociedades, Registro Mercantil y Central, asegurando un manejo óptimo y preciso.",
                "Con nosotros, tu contabilidad reflejará claridad y eficiencia."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "At Germán Sáez Consulting, we transform numbers into successful strategies.",
                "From corporate accounting and costs to detailed depreciation analysis.",
                "Our experience covers Corporate Tax, Commercial and Central Registry, ensuring optimal and precise management.",
                "With us, your accounting will reflect clarity and efficiency."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Bei der Steuerberatung Germán Sáez verwandeln wir Zahlen in erfolgreiche Strategien.",
                "Von der Buchhaltung und Kostenrechnung bis hin zur detaillierten Abschreibungsanalyse.",
                "Unsere Erfahrung umfasst die Körperschaftsteuer, das Handels- und Zentralregister und gewährleistet eine optimale und präzise Verwaltung.",
                "Mit uns spiegelt Ihre Buchhaltung Klarheit und Effizienz wider."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    },
    {
        nombre: "2BOKA2 SPAR",
        logo: "/img-empresas/2boka2-spar.webp",
        url: "https://share.google/tvZTuxKsnsSo1aXzI",
        es: {
            descripcion: [
                "2BOKA2 - SPAR City Titos es un supermercado de proximidad que ofrece una amplia variedad de productos, servicio a domicilio y soluciones para eventos como food truck y catering para fiestas y celebraciones.",
                "Destaca por su cercanía, comodidad y calidad.",
                "Un colaborador ideal para complementar la experiencia de vuestros jugadores de pádel, tanto en su rutina diaria como en la organización de eventos y torneos."
            ],
            btnText: "¡RESERVA YA!"
        },
        en: {
            descripcion: [
                "2BOKA2 - SPAR City Titos is a local supermarket offering a wide variety of products, delivery service, and solutions for events such as food trucks and catering for parties and celebrations.",
                "Stands out for its proximity, convenience, and quality.",
                "An ideal partner to complement the experience of your pickleball players, both in their daily routine and when organizing events and tournaments."
            ],
            btnText: "BOOK NOW!"
        },
        de: {
            descripcion: [
                "2BOKA2 - SPAR City Titos ist ein Nahversorgungsmarkt mit einer breiten Produktpalette, Lieferservice und Lösungen für Veranstaltungen wie Food Trucks und Catering für Partys und Feiern.",
                "Zeichnet sich durch Nähe, Bequemlichkeit und Qualität aus.",
                "Ein idealer Partner, um das Erlebnis Ihrer Pickleball-Spieler zu ergänzen, sowohl im Alltag als auch bei der Organisation von Events und Turnieren."
            ],
            btnText: "JETZT BUCHEN!"
        }
    },
    {
        nombre: "REY DE PALMERAS",
        logo: "/img-empresas/rey-de-palmeras.webp",
        url: "https://www.facebook.com/vicente.garciaparra.5",
        es: {
            descripcion: [
                "El Rey de las Palmeras es una empresa especializada en poda de palmeras y trabajos en altura con sede en Águilas.",
                "Contamos con un equipo profesional altamente capacitado y las herramientas adecuadas para garantizar un servicio seguro, eficiente y de calidad.",
                "Nos encargamos del mantenimiento y cuidado de palmeras, asegurando su salud y estética, además de realizar trabajos en altura en diversos entornos ¡Tu palmera en las mejores manos!"
            ],
            btnText: "¡PIDE MÁS INFO!"
        },
        en: {
            descripcion: [
                "El Rey de las Palmeras is a company specializing in palm pruning and height works based in Águilas.",
                "We have a highly trained professional team and proper tools to guarantee safe, efficient, and quality service.",
                "We take care of the maintenance and care of palm trees, ensuring their health and aesthetics, as well as carrying out high-altitude work in various environments. Your palm tree in the best hands!"
            ],
            btnText: "MORE INFO!"
        },
        de: {
            descripcion: [
                "El Rey de las Palmeras ist ein Unternehmen, das sich auf Palmenschnitt und Höhenarbeiten in Águilas spezialisiert hat.",
                "Wir verfügen über ein hochqualifiziertes professionelles Team und geeignete Ausrüstung, um einen sicheren, effizienten und qualitativ hochwertigen Service zu gewährleisten.",
                "Wir kümmern uns um die Pflege und Wartung von Palmen und sorgen für ihre Gesundheit und Ästhetik, sowie Höhenarbeiten in verschiedenen Umgebungen. Ihre Palme in den besten Händen!"
            ],
            btnText: "MEHR INFO!"
        }
    },
    {
        nombre: "QUIETUD",
        logo: "/img-empresas/quietud.webp",
        url: "http://instagram.com/quietudaguilas/",
        es: {
            descripcion: [
                "Quietud - David García Osteopatía Integral es un centro especializado en osteopatía y tratamiento integral del cuerpo, enfocado en aliviar dolores musculares y articulares, mejorar la movilidad y optimizar el rendimiento físico.",
                "A través de un enfoque personalizado, trabaja tanto la prevención como la recuperación de lesiones.",
                "Un colaborador ideal para vuestros clientes de pádel, ayudándoles a cuidarse, prevenir sobrecargas y mantenerse en las mejores condiciones para rendir al máximo en la pista."
            ],
            btnText: "¡INFÓRMATE YA!"
        },
        en: {
            descripcion: [
                "Quietud - David García Comprehensive Osteopathy is a specialized center for osteopathy and full body treatment, focused on relieving muscle and joint pain, improving mobility, and optimizing physical performance.",
                "Through a personalized approach, it works on both prevention and injury recovery.",
                "An ideal partner for your pickleball clients, helping them take care of themselves, prevent overloads, and stay in the best conditions to perform at their best on the court."
            ],
            btnText: "LEARN MORE!"
        },
        de: {
            descripcion: [
                "Quietud - David García Ganzheitliche Osteopathie ist ein Zentrum für Osteopathie und ganzheitliche Körperbehandlung, das sich auf die Linderung von Muskel- und Gelenkschmerzen, die Verbesserung der Mobilität und die Optimierung der körperlichen Leistung konzentriert.",
                "Durch einen personalisierten Ansatz werden sowohl Prävention als auch Verletzungsrehabilitation behandelt.",
                "Ein idealer Partner für Ihre Pickleball-Kunden, der ihnen hilft, auf sich zu achten, Überlastungen zu vermeiden und in bester Verfassung für maximale Leistung auf dem Platz zu bleiben."
            ],
            btnText: "MEHR ERFAHREN!"
        }
    },
    {
        nombre: "PINTAMAR",
        logo: "/img-empresas/pintamar.webp",
        url: "https://www.instagram.com/pintamardeco/",
        es: {
            descripcion: [
                "Pintamar Deco es una empresa especializada en pinturas, protección y decoración, con experiencia también en el sector náutico.",
                "Ofrece asesoramiento profesional y soluciones personalizadas para viviendas, negocios y embarcaciones, combinando calidad, conocimiento y acabados duraderos.",
                "Un gran colaborador para vuestros clientes de pádel, aportando soluciones tanto para el cuidado y mejora de sus espacios como para proyectos de reforma o mantenimiento."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "Pintamar Deco is a company specialized in paints, protection, and decoration, with experience also in the nautical sector.",
                "Provides professional advice and personalized solutions for homes, businesses, and boats, combining quality, knowledge, and durable finishes.",
                "A great partner for your pickleball clients, providing solutions for both the care and improvement of their spaces and renovation or maintenance projects."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Pintamar Deco ist ein auf Farben, Schutz und Dekoration spezialisiertes Unternehmen mit Erfahrung auch im Nautikbereich.",
                "Bietet professionelle Beratung und individuelle Lösungen für Wohnungen, Unternehmen und Boote und kombiniert Qualität, Fachwissen und dauerhafte Oberflächen.",
                "Ein großartiger Partner für Ihre Pickleball-Kunden, der Lösungen sowohl für die Pflege und Verbesserung ihrer Räume als auch für Renovierungs- oder Wartungsprojekte bietet."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    },
    {
        nombre: "ELENA ROJAS",
        logo: "/img-empresas/elena-rojas.webp",
        url: "https://share.google/YorkT0j2Pddk30XdS",
        es: {
            descripcion: [
                "Elena Rojas - Peluquería y Estética Unisex es un centro especializado en belleza y cuidado personal, que ofrece servicios de estética como manicura, pedicura, depilación, limpieza facial y maquillaje, además de peluquería con corte, color, recogidos y tratamientos de keratina.",
                "Un colaborador perfecto para vuestros clientes de pádel, ayudándoles a cuidar su imagen y bienestar tanto para su día a día como para eventos y ocasiones especiales."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "Elena Rojas - Unisex Hairdresser and Aesthetics is a specialized center for beauty and personal care, offering aesthetic services such as manicures, pedicures, hair removal, facial cleansing, and makeup, as well as hairdressing with cuts, colors, updos, and keratin treatments.",
                "A perfect partner for your pickleball clients, helping them care for their image and well-being for both their day-to-day and special events and occasions."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Elena Rojas - Unisex-Friseur und Ästhetik ist ein spezialisiertes Zentrum für Schönheit und Körperpflege, das ästhetische Dienstleistungen wie Maniküre, Pediküre, Haarentfernung, Gesichtsreinigung und Make-up sowie Friseurleistungen mit Schnitt, Farbe, Hochsteckfrisuren und Keratin-Behandlungen anbietet.",
                "Ein perfekter Partner für Ihre Pickleball-Kunden, der ihnen hilft, ihr Image und Wohlbefinden für den Alltag sowie für besondere Ereignisse zu pflegen."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    },
    {
        nombre: "RPASCORSO DJI",
        logo: "/img-empresas/rpascorso-dji.webp",
        url: "https://rpascorso.com/",
        es: {
            descripcion: [
                "Somos la escuela Nº 1 en España, desde 2015 formando pilotos de drones y pilotos de aviación civil en más de 15 países en todo el mundo.",
                "Os presentamos nuestros \"DRONES DE FABRICACIÓN ESPAÑOLA\" y además con algo primordial para todo agricultor: UN RÁPIDO SERVICIO POST-VENTA.",
                "¡Siempre contigo para ayudarte a despegar en este mundo de la agricultura de precisión!"
            ],
            btnText: "¡PIDE MÁS INFO!"
        },
        en: {
            descripcion: [
                "We are the No. 1 school in Spain, training drone pilots and civil aviation pilots since 2015 in over 15 countries worldwide.",
                "We present our 'SPANISH MANUFACTURED DRONES' along with something essential for every farmer: A FAST AFTER-SALES SERVICE.",
                "Always with you to help you take off in the world of precision agriculture!"
            ],
            btnText: "MORE INFO!"
        },
        de: {
            descripcion: [
                "Wir sind die Nummer-1-Schule in Spanien, die seit 2015 Drohnenpiloten und Zivilluftfahrtpiloten in über 15 Ländern weltweit ausbildet.",
                "Wir präsentieren unsere 'SPANISCH HERGESTELLTEN DROHNEN' sowie etwas Wesentliches für jeden Landwirt: EINEN SCHNELLEN KUNDENDIENST.",
                "Immer mit Ihnen, um Ihnen beim Abflug in die Welt der Präzisionslandwirtschaft zu helfen!"
            ],
            btnText: "MEHR INFO!"
        }
    },
    {
        nombre: "VINTAGE STUDIO",
        logo: "/img-empresas/vintage-studio.webp",
        url: "https://www.vintage-estudiografico.com/",
        es: {
            descripcion: [
                "Vintage Estudio Gráfico - Andrés Martínez es un estudio especializado en diseño gráfico e imagen corporativa, ofreciendo soluciones creativas y personalizadas para empresas y profesionales.",
                "Realiza rotulación de vehículos, cartelería, letras corpóreas, ropa personalizada, vinilos, lonas, tarjetas, folletos y todo tipo de material publicitario.",
                "Un colaborador ideal para vuestros clientes de pádel, ayudándoles a potenciar su marca personal, negocios o eventos con una imagen profesional y atractiva."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "Vintage Graphic Studio - Andrés Martínez is a studio specialized in graphic design and corporate branding, offering creative and personalized solutions for businesses and professionals.",
                "Performs vehicle lettering, signs, channel letters, custom clothing, vinyls, banners, cards, brochures, and all kinds of advertising material.",
                "An ideal partner for your pickleball clients, helping them enhance their personal brand, business, or events with a professional and attractive image."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Vintage Grafikstudio - Andrés Martínez ist ein auf Grafikdesign und Corporate Branding spezialisiertes Studio, das kreative und individuelle Lösungen für Unternehmen und Fachleute anbietet.",
                "Fertigt Fahrzeugbeschriftungen, Schilder, Leuchtbuchstaben, Berufskleidung, Folierungen, Banner, Visitenkarten, Broschüren und alle Arten von Werbematerial.",
                "Ein idealer Partner für Ihre Pickleball-Kunden, der ihnen hilft, ihre persönliche Marke, ihr Unternehmen oder ihre Veranstaltungen mit einem professionellen und attraktiven Image zu stärken."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    },
    {
        nombre: "SPORTTIA",
        logo: "/img-empresas/sporttia.webp",
        url: "https://www.sporttia.com/es/inicio/",
        es: {
            descripcion: [
                "Reservas online de pistas o entradas a piscinas.",
                "Reduce el número de llamadas de los usuarios para reservar.",
                "Pago de tasas online a través de tarjeta de crédito o monedero.",
                "Evita cobros al contado o en efectivo.",
                "Inscripciones online de actividades o escuelas deportivas.",
                "Suprime aglomeraciones para formalizar una inscripción y difunde tus eventos."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "Online reservations for courts or pool tickets.",
                "Reduces the number of calls from users to book.",
                "Online fee payment via credit card or wallet.",
                "Avoids cash payments.",
                "Online registration for activities or sports schools.",
                "Eliminates crowds for registrations and spreads your events."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Online-Reservierungen für Plätze oder Schwimmbadbadezugänge.",
                "Reduziert die Anzahl der Anrufe von Benutzern zur Buchung.",
                "Online-Gebührenzahlung per Kreditkarte oder Geldbörse.",
                "Vermeidet Barzahlungen.",
                "Online-Anmeldung für Aktivitäten oder Sportschulen.",
                "Eliminiert Menschenansammlungen bei Anmeldungen und verbreitet Ihre Veranstaltungen."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    },
    {
        nombre: "TURISMO JOVEN",
        logo: "/img-empresas/turismo-joven.webp",
        url: "http://WWW.TURISMOJOVEN.ES",
        es: {
            descripcion: [
                "Turismo Joven es una agencia receptiva especializada en programas de inmersión lingüística y campamentos náuticos, ofreciendo experiencias educativas y de ocio para niños y jóvenes.",
                "Combina aprendizaje, deporte y diversión en entornos seguros y organizados por un equipo profesional.",
                "Un excelente colaborador para vuestros clientes de pádel, brindando a sus familias opciones formativas y deportivas para los más pequeños, especialmente en periodos vacacionales."
            ],
            btnText: "¡PIDE INFO AQUÍ!"
        },
        en: {
            descripcion: [
                "Turismo Joven is an inbound agency specializing in linguistic immersion programs and nautical camps, offering educational and leisure experiences for children and youngsters.",
                "Combines learning, sports, and fun in safe environments organized by a professional team.",
                "An excellent partner for your pickleball clients, providing their families with educational and sports options for the little ones, especially during holiday periods."
            ],
            btnText: "INFO HERE!"
        },
        de: {
            descripcion: [
                "Turismo Joven ist eine auf Sprachimmersionsprogramme und nautische Camps spezialisierte Inbound-Agentur, die Bildungs- und Freizeiterlebnisse für Kinder und Jugendliche anbietet.",
                "Kombiniert Lernen, Sport und Spaß in sicheren, von einem professionellen Team organisierten Umgebungen.",
                "Ein ausgezeichneter Partner für Ihre Pickleball-Kunden, der ihren Familien Bildungs- und Sportoptionen für die Kleinsten bietet, besonders in der Urlaubszeit."
            ],
            btnText: "HIER INFO ANFORDERN!"
        }
    }
];
